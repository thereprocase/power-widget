...
def accuracy(a, voltage, power, mode="logging_1s", calibrated=True):
...
def main():
    ...
    for name, mode in a["modes"].items():
        ...
        modes[name] = {
            ...
            "report_window_s": mode.get("software_report_window_s", period_s),
            "noise_scope": mode.get("noise_scope", "one hardware conversion/averaging window"),
        }
    ...
    loss_rows = [path_loss(a, i) for i in sorted(set([5, analysis_current, design_current]))]
    ...
    assert all(row["within_75_percent_of_1w_shunt_allocation"] for row in scenarios), "Laptop current scenario exceeds shunt allocation"
    ...
    lines = ["# Calculated budgets", "", "Generated by `python tools/budgets.py` from `analysis/assumptions.json`.", "",
             ...
             f'Current rule: verified OEM maximum + 0.5 A, followed by 20% design margin. **The documented laptop-roster maximum is {req["verified_oem_max_current_a"]:g} A. The selected continuous design target is {analysis_current:g} A; hardware qualification is pending.**', "",
             "## One-second logging DC error allocation", "",
             "| Voltage | Power | Current | Shunt signal | Calculated error bound | Target |", "|---|---|---|---|---|---|"]
    for x in selected:
        if x["mode"] == "logging_1s":
    ...
    lines += ["", f'The sweep evaluates {len(grid):,} points. Largest precision allocation at ≥5 W: **{worst_normal["power_error_percent"]:.3f}%** ({worst_normal["voltage_v"]:g} V, {worst_normal["power_w"]:g} W). Below 5 W: **{worst_low["power_error_percent"]:.3f}%** ({worst_low["voltage_v"]:g} V, {worst_low["power_w"]:g} W).', "",
              "## Timing and noise", "", "| Mode | Sensor window | Fresh updates | Report window | Noise allocation |", "|---|---|---|---|---|"]
    for name, m in modes.items():
        lines.append(f'| {name} | {m["nominal_window_s"]*1000:.3f} ms | {m["nominal_fresh_updates_hz"]:.2f}/s | {m["report_window_s"]:g} s | ±{m["absolute_noise_allocation_uv"]:g} µV |')
    lines += ["", f'Trend-mode allocation at 28 V / 0.5 W: **{sensitivity["trend_28v_half_watt"]["power_error_percent"]:.3f}%**; it does not meet the low-power precision target.', "",
              "Logging mode uses frequent 17.28 ms sensor windows and one-second software aggregation. Its ±1 µV noise allocation applies to the final one-second average and requires bench validation; do not assume correlated errors average away. The older precision mode remains a calibration/diagnostic option. These DC budgets do not bound transient energy error.", "",
