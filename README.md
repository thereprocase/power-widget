...
>
> Keep all lettering clear of conductors: never run a wire through a net name, reference, value, pin label or note. Place ordinary wire labels above or beside the wire with visible clearance; put boxed net tags on short annotation stubs in open space. Preserve the continuous main wire and use an explicit junction at each stub. Check the actual native export, not just the underlying coordinates or netlist. Visually inspect every functional section at readable magnification, including both ends of each connection, pin numbers, component values, junctions, crossings, polarity, symbol orientation and callouts. Then review the whole sheet for flow and consistency. Keep lettering clear of other lettering, symbol graphics (including LED arrows), and dashed graphical boundaries too. Add automated rendered-text/wire and text/text overlap checks where possible, and fix every collision before publishing.
